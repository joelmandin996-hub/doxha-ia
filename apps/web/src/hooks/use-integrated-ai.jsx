...
function mapUserMessage({ message }) {
	...
	// Strip out context block from history so the UI doesn't show the massive context string
	let content = textParts.join('\n');
	if (content.includes('CONTEXTE FINANCIER:\n') && content.includes('\n\nQUESTION:\n')) {
...
function useIntegratedAi() {
	...
	const sendMessage = useCallback(async (userMessage, images = [], contextData = null) => {
		setIsStreaming(true);
		...
		setMessages(prev => [
			...
			{
				role: 'user',
				content: userMessage, // UI only sees the pure question
				...(images.length > 0 && {
		...
		try {
			const formattedMessage = contextData 
				? `CONTEXTE FINANCIER:\n${contextData}\n\nQUESTION:\n${userMessage}`
				: userMessage;

			const response = await integratedAiClient.stream('/integrated-ai/stream', {
				body: { message: [{ text: formattedMessage, type: 'text' }] },
